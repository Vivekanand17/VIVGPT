...
# Step 1: Update framer-motion import
content = content.replace(
    'import { motion } from "framer-motion";',
    'import { motion, AnimatePresence } from "framer-motion";'
)

# Step 2: Add new lucide-react icons
content = content.replace(
    "  Calculator,\n} from \"lucide-react\";",
    "  Calculator,\n  Menu,\n  X,\n  Pin,\n  PinOff,\n  MoreHorizontal,\n  Trash2,\n  Pencil,\n} from \"lucide-react\";"
)
...
content = content.replace(
  ...
  const [pinnedThreads, setPinnedThreads] = useState(() => {
    try { const s = localStorage.getItem("pinned_threads"); return s ? JSON.parse(s) : []; }
    catch { return []; }
  });
...
# Step 4: Add refs and click handler
content = content.replace(
...
content = content.replace(
  ...
  const togglePin = (id) => {
    setPinnedThreads((prev) => {
      ...
      localStorage.setItem("pinned_threads", JSON.stringify(u)); return u;
    });
  };

  const startRenaming = (conv) => {
    setRenamingThread(conv.thread_id); setRenameValue(conv.title || "New Chat"); setMenuOpenThread(null);
  };
  ...
  const submitRename = async () => {
    ...
    setRenamingThread(null); setRenameValue("");
  };
    ...
    try {
      ...
      if (deleteConfirmThread === threadId) {
        ...
        if (rem.length > 0) await loadConversation(rem[0].thread_id);
          ...
          const nid = crypto.randomUUID(); setThreadId(nid);
          localStorage.setItem("thread_id", nid); setMessages([]); setShowWelcome(true);
        }
      }
      await loadConversations();
    } catch (e) { console.error(e); }
  ...
  const sortedConversations = useMemo(() => {
    const p = []; const u = []; const s = searchQuery.toLowerCase().trim();
    for (const c of conversations) {
      const t = (c.title || "New Chat").toLowerCase();
      if (s && !t.includes(s)) continue;
      if (pinnedThreads.includes(c.thread_id)) p.push(c); else u.push(c);
    }
    const sortFn = (a, b) => new Date(b.updated_at) - new Date(a.updated_at);
    p.sort(sortFn); u.sort(sortFn); return [...p, ...u];
  }, [conversations, pinnedThreads, searchQuery]);

  return ("""
)

# Step 6: Sidebar collapsible wrapper
content = content.replace(
...
# Step 8: Replace conversation list with sorted + features
content = content.replace(
...
# Step 9: Close sidebar wrapper div, add delete confirmation
content = content.replace(
    "</aside>\n\n        {/* Main chat area */}",
    """</div>
        </aside>
...
# Step 10: Add sidebar toggle button
content = content.replace(
...
# Step 11: Remove md:left-[270px] from input area (last occurrence)
idx = content.rfind("md:left-[270px]")
if idx > 0:
    content = content[:idx] + content[idx:].replace("md:left-[270px]", "", 1)
    print("Step 11: Removed md:left-[270px]")

print(f"Patched: {original_len} -> {len(content)} bytes (+{len(content)-original_len})")
src.write_text(content, "utf-8")
print("DONE! ChatSection.jsx updated successfully.")
</｜｜DSML｜｜parameter>
</｜｜DSML｜｜invoke>
</｜｜DSML｜｜tool_calls>
